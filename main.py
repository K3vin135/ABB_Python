...
def send_message():
    global boxes_data, abb_data, conn

    # Posición del ABB
    abb_x, abb_y, abb_z = abb_data[0]['x'], abb_data[0]['y'], abb_data[0]['z']

    # Calcular la distancia de cada caja al ABB
    if len(boxes_data) >= 1:
        distances = []
        for box in boxes_data:
            distance = ((box['x'] - abb_x)**2 + (box['y'] - abb_y)**2 )**0.5
            distances.append((distance, box))
        
        # Ordenar las cajas por distancia
        distances.sort(key=lambda x: x[0])
        print('Distancias:', distances)
        
        # Usar las coordenadas de la caja más cercana como caja 1
        box1 = distances[0][1]
        x1, y1, z1 = box1['x']*1000, box1['y']*1000, 170
        
        # Usar las coordenadas de la segunda caja más cercana como caja 2
        if len(distances) > 1:
            box2 = distances[1][1]
            x2, y2, z2 = box2['x']*1000, box2['y']*1000, 170
        else:
            # Si no hay dos cajas detectadas, enviar (0, 0, 0)
            x2, y2, z2 = 0, 0, 0
    else:
    ...
    message = f"{x1},{y1},{z1},{x2},{y2},{z2},{Moverobot},{numBox}\n"
